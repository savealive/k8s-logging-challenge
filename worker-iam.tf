  ...
  assume_role_policy = <<POLICY
{
  ...
  "Statement": [
    ...
    },
    {
      ...
      "Effect": "Allow",
      "Principal": {
        "AWS": "${aws_iam_role.kube-node.arn}"
      },
      "Action": "sts:AssumeRole"
    }
  ]
...
resource "aws_iam_policy" "external_dns" {
  name        = "ExternalDNSUsePolicy"
  path        = "/"
  description = "Allow External DNS k8s service manage hosted zones"
...
resource "aws_iam_policy" "alb_ingress_policy" {
  ...
  path        = "/"
  description = "Allows Kube nodes to create ALB loadbalancer in order to run alb-ingress"
...
resource "aws_iam_role_policy_attachment" "kube-node-AmazonRoute53ExternalDNSUsage" {
  policy_arn = "${aws_iam_policy.external_dns.arn}"
  role       = "${aws_iam_role.alb_role.name}"
}

resource "aws_iam_role_policy_attachment" "alb-ingress-policy" {
